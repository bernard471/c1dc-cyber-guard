import { EmailHackReport } from '@/services/reports/types';
import { Card, CardContent } from "@/components/ui/card";
import { Mail, Calendar, ChevronRight, Download, AlertCircle, Shield, AlertTriangle } from 'lucide-react';
...
interface AdminEmailHackReportCardProps {
  report: EmailHackReport;
  isExpanded: boolean;
  onToggle: () => void;
...
export const AdminEmailHackReportCard = ({ 
  report, 
  isExpanded, 
  onToggle,
  getStatusColor,
  onStatusUpdate
}: AdminEmailHackReportCardProps) => {
  ...
  return (
    <>
    <Card className="overflow-hidden mb-6 border-l-4 border-l-emerald-500 hover:shadow-lg transition-all duration-200">
            ...
            <div>
              ...
              <div className="mt-3 text-sm text-gray-600 space-y-1">
                <p className="flex items-center gap-2">
                  ...
                  {new Date(report.dateOfHack).toLocaleDateString()}
                </p>
                <p className="flex items-center gap-2">
                  <Shield className="w-4 h-4 text-gray-400" />
                  {report.emailProvider} - {report.hackMethod}
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="bg-white p-4 rounded-lg shadow-sm">
                ...
                <dl className="space-y-3 text-sm">
                  {[
                    { label: 'Email Address', value: report.emailAddress },
                    { label: 'Email Provider', value: report.emailProvider },
              ...
              <div className="bg-white p-4 rounded-lg shadow-sm">
                ...
                <dl className="space-y-3 text-sm">
                  {[
                    { label: 'Emails Compromised', value: report.emailsCompromised ? 'Yes' : 'No' },
                    { label: 'Contacts Affected', value: report.contactsAffected ? 'Yes' : 'No' },
                    { label: 'Password Changed', value: report.passwordChanged ? 'Yes' : 'No' },
                    { label: 'Recovery Attempted', value: report.recoveryAttempted ? 'Yes' : 'No' },
                    { label: 'Linked Accounts Affected', value: report.linkedAccountsAffected ? 'Yes' : 'No' },
                    { label: 'Recovery Email Compromised', value: report.recoveryEmailCompromised ? 'Yes' : 'No' },
              ...
              <div className="bg-white p-4 rounded-lg shadow-sm">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <button
                    ...
                    className="flex items-center gap-2 px-3 py-1.5 text-sm text-emerald-600 hover:text-emerald-700 bg-emerald-50 hover:bg-emerald-100 rounded-full transition-colors duration-200"
                  >
